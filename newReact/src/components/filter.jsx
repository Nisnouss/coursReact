...
function FilterList({items}){
    ...
    const handleSearch = (e) => {
        ...
        setSearch(val) // Mise à jour immédiate (pour que l'input reste fluide)

        startTransition(() => {
            // Mise à jour différé (le filtrage lourd se fera en tâche de fond)
            setDeferredSearch(val)
        })
    }

    // Le filtre ne sera recalculé QUE si 'items' ou 'search' change
    const itemsFilters = useMemo(() => {
        console.log("Calcul lourd en cours ...")
        return items.filter(item => item.name.includes(deferredSearch))
    }, [items, deferredSearch])

    return(
        <>
            {/* Input lié à la recherche et affichage de itemsFilters */}
